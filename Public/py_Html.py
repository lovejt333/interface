...
shanghai = '''<div class="row " style="margin:60px">
        ...
        <div class="btn-group" role="group" aria-label="...">
            ...
            <button type="button" id="check-danger" class="btn btn-danger">错误用例</button>
...
def relust(titles, starttime, endtime, passge, fail, id, name, coneent, url, meth, yuqi, json, relust, exceptions,
           weizhi):
    if type(name) == list:
        relus = ' '
        for i in range(len(url)):
            if relust[i] == "pass":
                ...
                clazz = 'error'
            relus += (
                ceshixiangqing(clazz, id[i], name[i], coneent[i], url[i], meth[i], yuqi[i], json[i], relust[i]))
        text = title(titles) + connent + shouye(starttime, endtime, passge, fail, exceptions,
                                                weizhi) + shanghai + relus + weibu
    else:
        text = title(titles) + connent + shouye(starttime, endtime, passge, fail, exceptions,
                                                weizhi) + shanghai + ceshixiangqing(id, name, coneent, url, meth,
                                                                                    yuqi, json, relust) + weibu
    return text


def createHtml(filepath, titles, starttime, endtime, passge, fail, id, name, coneent, url, meth, yuqi, json,
               relusts, exceptions, weizhi):
    texts = relust(titles, starttime, endtime, passge, fail, id, name, coneent, url, meth, yuqi, json, relusts,
                   exceptions, weizhi)
